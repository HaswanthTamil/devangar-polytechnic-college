        ...
        <div className="flex items-center space-x-4 flex-1"> {/* Added flex-1 to grow */}
          ...
          <div className="overflow-hidden h-6 relative flex-1">
            {data.map((item, idx) => (
              <div
                key={idx}
                className={`absolute inset-x-0 transition-all duration-700 ease-in-out transform flex items-center h-full ${idx === currentIndex ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0 pointer-events-none'
                  }`}
              >
